...
;; check if linux on WSL, copied from
;; https://www.reddit.com/r/emacs/comments/1e9dmb3/comment/ledyp2j/
(defun wslp ()
  "Return t if Emacs is running in WSL2, nil otherwise."
  (and
   ;; Check if the system is Linux
   (eq system-type 'gnu/linux)
   ;; I have a different file, the other conditions should be enough
...
(if (raiszo/my-system-type-is-darwin)
  ...
  (progn
    (set-face-attribute 'default nil :family "Cascadia Code NF" :height 110)
    (set-face-attribute 'italic nil :family "Cascadia Code NF" :height 110 :slant 'italic :underline nil)))

#+end_src

** Ansi support in compilation buffers
...
#+end_src
** Incremental completions and narrowing selections with Helm
#+begin_src emacs-lisp
(use-package helm
  :ensure t
  :bind (("M-x" . helm-M-x)
...
(use-package dashboard
  ...
  (dashboard-startup-banner "~/.emacs.d/images/yorha-logo-light-010.png")
  :config
...
(use-package helm-projectile
  ...
  :hook (after-init-hook . helm-projectile-mode))
  ...
  ;; (define-key projectile-mode-map [remap projectile-search] #'helm-projectile-rg)
...
(use-package lsp-mode
  ...
  (lsp-enable-snippet nil)	  ;disabled until yasnippet comes back
  (lsp-signature-auto-activate nil)
  (lsp-completion-use-last-result t)
  ;; (lsp-clients-typescript-max-ts-server-memory 3072)
  ;; (lsp-clients-typescript-prefer-use-project-ts-server t)
  (lsp-idle-delay 0.200)
  (read-process-output-max (* 1024 1024 8)) ;; 1mb
...
(use-package lsp-ui
  ...
  (lsp-ui-imenu-enable t)		;nil?
  (lsp-ui-imenu-kind-position 'top)
  (lsp-ui-sideline-enable nil))

;; maybe add helm-lsp

(defun my-typescript-company-idle-delay ()
  "Suppress automatic completion immediately after `{` and `,`."
  (unless (and (derived-mode-p 'typescript-ts-mode 'tsx-ts-mode)
               (memq (char-before) '(?{ ?, ?\[)))
    0.2))
...
(use-package company
  ...
  (company-idle-delay #'my-typescript-company-idle-delay))
...
;; TODO documentation via quickhelp is kind of laggy for typescript
...
(use-package dap-mode
  ...
  ;; STM32Cube's VS Code extension ships a standalone Node DAP adapter.
  ;; Project and target settings stay in .vscode/launch.json; this provider
  ;; only tells dap-mode how to start the adapter.  Prefix PATH for this
  ;; process so the adapter can use ST's `cube' wrapper to resolve its GDB,
  ;; ST-LINK GDB server, and STM32CubeProgrammer bundles.
   ...
   (lambda (conf)
     (plist-put
      ...
      (list
       "env"
       (concat
        "PATH=/home/xyz/.vscode/extensions/stmicroelectronics.stm32cube-ide-core-1.4.0-linux-x64/resources/binaries/linux/x86_64"
        path-separator
        (getenv "PATH"))
       "node"
       "/home/xyz/.vscode/extensions/stmicroelectronics.stm32cube-ide-debug-stlink-gdbserver-1.4.0/lib/adapter/STLinkDebugTargetAdapter.js")))))
#+end_src
...
*** astro
Dev put the autoload back in this commit https://git.isincredibly.gay/srxl/astro-ts-mode/commit/3ece1b9f617baeab6b9b2f997cb3b10863466d59
that is why I get =Error loading autoloads: (void-function treesit-ready-p)= :c
...
(use-package lsp-sourcekit
  ...
  :after lsp-mode
  :config
  (setq lsp-sourcekit-executable "~/.local/share/swiftly/toolchains/6.3.3/usr/bin/sourcekit-lsp"))
#+end_src
** cmake
#+begin_src emacs-lisp
...
(use-package go-ts-mode
  :mode ("\\.go\\'")
  :hook (go-ts-mode . (lambda () (setq tab-width 4))))
  ...
  (setq org-roam-capture-templates
	'(;; bibliography note template
          ("b" "bibliography reference" plain "%?"
           ...
           :unnarrowed t))))
#+end_src
** COMMENT Presentations with org-present
#+begin_src emacs-lisp
